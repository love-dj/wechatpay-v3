...
namespace Love\WeChatPayV3\Kernel;

use GuzzleHttp\Exception\GuzzleException;
use Illuminate\Support\Arr;
use Illuminate\Support\Carbon;
use Love\WeChatPayV3\Kernel\Exceptions\DecryptException;
use Love\WeChatPayV3\Kernel\Exceptions\InvalidArgumentException;
use Love\WeChatPayV3\Kernel\Exceptions\RuntimeException;
use Love\WeChatPayV3\Kernel\Exceptions\SignInvalidException;
use Love\WeChatPayV3\Kernel\Utils\AesUtil;
use Love\WeChatPayV3\Service\Certificate\Client;
...
class Certificate
{
    ...
    public function getPublicKey($serialNo)
    {
        ...
        $certificateClient = $this->app['certificate'];
        $certificates = collect(Arr::get($certificateClient->all(), 'data'));
        $certificate = $certificates->where('serial_no', '=', $serialNo)->toArray();
        if (empty($certificate)) {
            throw new SignInvalidException('证书序列号不存在于可用的证书列表中');
        }
        $aesKey = $this->app->config->get('aes_key');
        $publicKey = $this->decryptCertificate(Arr::get($certificate[0], 'encrypt_certificate'), $aesKey);
        if ($client != null && $client instanceof Redis) {
            $client->set($this->getSerialNoCacheKey($mchId), $serialNo, $ttl);
            $client->set($this->getPublicKeyCacheKey($serialNo), $publicKey, $ttl);
        }
